...
% UAV_position_controller_AFBC - 纯净反步法位置环 (修复倒退起飞现象)
    ...
    % ================= 修复倒退起飞核心 =================
    % 剥离出反馈纠偏速度
    feedback_v = K1_kin * tanh(z1_sat) + K2_kin * (abs(z1_sat).^(2*beta-1)) .* sign(z1_sat);
    ...
    F_d = m * alpha_p_dot - z1 - K1_dyn * tanh(z2) - sigma * tanh(z2/0.1) - K2_dyn * diag(abs(z2).^(2*beta-1)) *sign(z2);
