% =======================================================================ID==
% 多智能体协同控制系统 - 终极算法对比框架 (AFBC vs BSC_DSC vs PID)
% 生成论文核心图表：误差收敛、局部轨迹、综合指标(IAE/ECI/MaxErr)、内环性能
% =========================================================================
clc; clear; close all;

algo_list = {'ET_PID', 'ET_DSC', 'ET_SAFBC'};
algo_names = {' ET_PID ', 'ET_DSC ', 'ET_SAFBC'};
colors = {'#B2E6FD', '#FBE79E', '#71B8ED'}; % PID红, BSC绿, AFBC蓝
...
for algo_idx = 1:length(algo_list)
    ...
    for k = 1:num_uavs
        ...
        uav_ctrl_state{k}.alpha_p_f = [0;0;0]; uav_ctrl_state{k}.alpha_a_f = [0;0;0];
    ...
    % === 【新增】初始化相对编队误差 (多智能体一致性) ===
    uav12_rel_err_history = zeros(1, N); % UAV1 与 UAV2 的相对误差
    usv83_rel_err_history = zeros(1, N); % USV8 与 UAV3 的跨域相对误差
    
    % 计算 t=0 时刻的初始相对误差
    [~, vel_d_0, p_ratio_0] = get_trajectory_state(0, plan, maneuver.v_c, maneuver.R_c);
    ts_uav_0 = uav_spacing_default + (s_uav_new - uav_spacing_default) * p_ratio_0;
    ts_usv_0 = usv_spacing_default + (s_usv_new - usv_spacing_default) * p_ratio_0;
    psi_form_0 = atan2(vel_d_0(2), vel_d_0(1));
    R_z_form_0 = [cos(psi_form_0), -sin(psi_form_0); sin(psi_form_0), cos(psi_form_0)];
    % t=0 误差计算
    uav12_rel_err_history(1) = norm((uav_state_history(1:2, 1, 1) - uav_state_history(1:2, 1, 2)) - R_z_form_0 * [0; -ts_uav_0]);
    usv83_rel_err_history(1) = norm(([usv_state.x(8); usv_state.y(8)] - uav_state_history(1:2, 1, 3)) - R_z_form_0 * [0; 1 * ts_usv_0]);
    % ==============================================
...
%             
%             % 3. 使用 usv_comm_state{j}.p_smooth 结合无噪音的理想航向矩阵计算目标点
...
%             psi_d = atan2(vd_usv(2), vd_usv(1)); speed_d = norm(vd_usv);
            ...
            usv_tau_history(:, i+1, j) = [tau(1); tau(3)]; % 【记录控制输入】
            ...
            % === 【新增】计算当前时刻的相对编队误差 ===
            % 1. UAV1 与 UAV2 的内部同构一致性误差
            p_uav1 = uav_state_history(1:2, i+1, 1);
            p_uav2 = uav_state_history(1:2, i+1, 2);
            % UAV1偏移量索引(k-2)=-1，UAV2偏移量索引=0，相对期望矢量 = 领航者切线系下的 [0; -1 * 间距]
            expected_rel_12 = R_z_form * [0; -target_spacing_uav]; 
            uav12_rel_err_history(i+1) = norm((p_uav1 - p_uav2) - expected_rel_12);
            
            % 2. USV8 与 UAV3 的跨域协同一致性误差
            p_usv8 = [usv_state.x(8); usv_state.y(8)];
            p_uav3 = uav_state_history(1:2, i+1, 3);
            % USV8对应的局部领航者是UAV3。USV8相对UAV3的索引差 s_idx = 8-7 = 1
            expected_rel_83 = R_z_form * [0; 1 * target_spacing_usv]; 
            usv83_rel_err_history(i+1) = norm((p_usv8 - p_uav3) - expected_rel_83);
            % ==========================================
        end
    end
    ...
    % === 【新增】封存相对编队误差数据 ===
    Results.(TEST_ALGO).uav12_rel_err = uav12_rel_err_history;
    Results.(TEST_ALGO).usv83_rel_err = usv83_rel_err_history;
    % ==================================
end
fprintf('\n仿真全部结束，正在生成综合对比图表...\n');
...
% =========================================================================
% 【核心图表】：综合性能量化对比 (IAE, RMSE, Max Error)
% =========================================================================
...
algo_colors = {'#B2E6FD', '#FBE79E', '#71B8ED'}; % PID, BSC, AFBC

for sub = 1:3
    subplot(1, 3, sub); hold on;
    
    % 组装数据矩阵 [UAV_PID, UAV_BSC, UAV_AFBC; USV_PID, USV_BSC, USV_AFBC]
    data = [Results.ET_PID.(metrics{sub*2-1}), Results.ET_DSC.(metrics{sub*2-1}), Results.ET_SAFBC.(metrics{sub*2-1});
            Results.ET_PID.(metrics{sub*2}),   Results.ET_DSC.(metrics{sub*2}),   Results.ET_SAFBC.(metrics{sub*2})];
    ...
    % 设置纵轴余量，防止数字标签超出格子
    y_max = max(data(:));
    ylim([0, y_max * 1.25]); % 统一预留25%的高度给标签
...
for sub = 1:4
    ...
    title(titles_uav{sub}); xlabel('Time (s)'); xlim([0, 100]); % 取前100s重点展示瞬态与抗扰
    if sub == 1; legend(algo_names, 'Location', 'best'); end
end

% =========================================================================
% 【核心图表 5】：内环性能对比 (USV 8 艏向与偏航力矩)
% =========================================================================
...
subplot(2, 1, 2); hold on; grid on; box on;
for i = 1:3; plot(time_vec, Results.(algo_list{i}).usv8_tau(2, :), 'Color', colors{i}, 'LineWidth', 1.0); end % tau(2,:) 是偏航力矩
title('最外侧 USV 8: 偏航力矩 \tau_r (N·m)'); xlabel('Time (s)');
% =========================================================================
% 【核心图表 6】：多智能体相对编队误差 (协同一致性分析)
% =========================================================================
figure('Name', '多智能体相对编队误差 (一致性)', 'Color', 'w', 'Position', [450, 150, 950, 650]);

% 子图 1: UAV 同构协同内部相对误差
subplot(2,1,1); hold on; grid on; box on;
for i = 1:3
    plot(time_vec, Results.(algo_list{i}).uav12_rel_err, 'Color', colors{i}, 'LineWidth', 1.5, 'DisplayName', algo_names{i});
end
title('同构协同: UAV 1 与 UAV 2 的相对队形误差 ||p_{u,1} - p_{u,2} - \Delta_{1,2}||', 'FontSize', 11, 'FontWeight', 'bold');
xlabel('Time (s)'); ylabel('Relative Error (m)');
legend('Location', 'northeast');

% 设定局部放大图观察稳态与转弯处的协同能力 (如不需要可注释)
axes('Position',[0.55 0.7 0.3 0.15]); box on; grid on; hold on;
for i = 1:3
    plot(time_vec, Results.(algo_list{i}).uav12_rel_err, 'Color', colors{i}, 'LineWidth', 1.5);
end
xlim([130, 230]); ylim([0, 2]); % 观察第二个大曲率转弯阶段
title('转弯机动段局部放大');

% 子图 2: USV-UAV 跨域协同相对误差
subplot(2,1,2); hold on; grid on; box on;
for i = 1:3
    plot(time_vec, Results.(algo_list{i}).usv83_rel_err, 'Color', colors{i}, 'LineWidth', 1.5, 'DisplayName', algo_names{i});
end
title('跨域协同: USV 8 与局部领航者 UAV 3 的相对队形误差 ||p_{s,8} - p_{u,3} - \Delta_{8,3}||', 'FontSize', 11, 'FontWeight', 'bold');
xlabel('Time (s)'); ylabel('Relative Error (m)');
legend('Location', 'northeast');

% 跨域协同放大图 (受风浪和事件触发影响，差距将非常显著)
axes('Position',[0.55 0.22 0.3 0.15]); box on; grid on; hold on;
for i = 1:3
    plot(time_vec, Results.(algo_list{i}).usv83_rel_err, 'Color', colors{i}, 'LineWidth', 1.5);
end
xlim([130, 230]); ylim([0, 3]); 
title('转弯机动段局部放大');
% ========================= 核心辅助函数 =========================
